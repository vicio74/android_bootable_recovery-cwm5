...
{
    LOGI("Finding partition: %s\n", name);
    if (g_mtd_state.partitions != NULL) {
        int i;
        for (i = 0; i < g_mtd_state.partitions_allocd; i++) {
            MtdPartition *p = &g_mtd_state.partitions[i];
            LOGI("Partition: %u, name %s\n", p->device_index, p->name );
        ...
        size_t *total_size, size_t *erase_size, size_t *write_size)
{
    ...
    //sprintf(mtddevname, "/dev/block/mtdblock%d", partition->device_index);
    ...
    LOGI("Partinfo: %s, size: %u, erasesize: %u, writesize: %u\n", mtddevname, mtd_info.size, mtd_info.erasesize, mtd_info.writesize);
    return 0;
}
